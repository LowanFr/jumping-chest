        ...
        for (int j = player->sprite->DestR.x/TAILLE_TEXTURES - 1; j <= player->sprite->DestR.x/TAILLE_TEXTURES + 1 ; ++j) {
            ...
            if(i < world->map->nb_row && i>=0 && j < world->map->nb_col && j>=0){
                ...
                if (condCollision1 && condCollision2 && conCollision3) {
                    ...
                    if (DestR.x + DestR.w > player->sprite->DestR.x && //Si le côté droit du bloc est inférieur au coté gauche du player
                    DestR.x < player->sprite->DestR.x && //Si le côté gauche du player est inférieur au coté gauche du bloc
                    player->prec.x >= DestR.x + DestR.w) { //Si le côté gauche précédent est bien supérieur au coté droit du bloc
                        player->prec.x = DestR.x + DestR.w;
                        player->sprite->DestR.x = player->prec.x;
                    }

                    // Collision à gauche du block
                    else if (DestR.x < player->sprite->DestR.x + player->sprite->DestR.w && //Si le coté gauche du bloc est inférieur au coté droit du player
